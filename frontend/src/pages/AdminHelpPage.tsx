...
export default function AdminHelpPage() {
  ...
  return <>
    ...
    {(canExportUmkm || canExportProducts) && <section className="mb-6 rounded-2xl border border-sage-border bg-white p-5"><h2 className="flex items-center gap-2 font-extrabold"><Download className="h-5 w-5 text-forest"/>Ekspor data</h2><p className="mt-2 text-sm text-warm-gray">Unduh CSV untuk kebutuhan administrasi desa. File tidak memuat kata sandi, sesi, atau data keamanan internal.</p><div className="mt-4 flex flex-wrap gap-3">{canExportUmkm && <a href={apiUrl('/manage/umkms/export.csv')} className={secondaryButtonClass}><Store className="h-4 w-4"/>Ekspor UMKM</a>}{canExportProducts && <a href={apiUrl('/manage/products/export.csv')} className={secondaryButtonClass}><Package className="h-4 w-4"/>Ekspor Produk</a>}</div></section>}
    <section className="grid gap-4 md:grid-cols-2" aria-label="Topik panduan">
      {topics.map(([title, description], index) => <article key={title} className="rounded-2xl border border-sage-border bg-white p-5"><h2 className="flex items-center gap-2 font-extrabold text-charcoal">{index === 0 ? <KeyRound className="h-5 w-5 text-forest"/> : index === 4 ? <MapPin className="h-5 w-5 text-forest"/> : <BookOpen className="h-5 w-5 text-forest"/>}{title}</h2><p className="mt-2 text-sm leading-6 text-warm-gray">{description}</p></article>)}
    </section>
  </>;
}
