...
import { ArrowLeft, Code2, HeartHandshake, Landmark, MapPin, Sparkles, Users } from 'lucide-react';
import { Link } from 'react-router';
import PublicPageShell from '../components/layout/PublicPageShell';
import { usePageMetadata } from '../lib/seo';

const contributions = [
  { icon: Code2, title: 'Digitalisasi Potensi Desa', description: 'Menghadirkan LoningMaju sebagai ruang digital untuk memperkenalkan UMKM dan produk lokal Desa Loning.' },
  { icon: HeartHandshake, title: 'Kolaborasi Bersama Warga', description: 'Membangun solusi melalui pendekatan partisipatif agar teknologi tetap relevan dengan kebutuhan masyarakat.' },
  { icon: Sparkles, title: 'Dampak yang Berkelanjutan', description: 'Menyusun platform yang dapat terus dimanfaatkan sebagai media informasi dan promosi setelah masa pengabdian.' },
];
    ...
    <PublicPageShell>
      <article className="bg-cream-bg text-charcoal antialiased">
        <header className="relative overflow-hidden border-b border-sage-border bg-forest px-5 py-20 text-cream-tint md:py-28">
          <div className="absolute -right-24 -top-24 h-80 w-80 rounded-full bg-terracotta/20 blur-3xl" aria-hidden="true" />
          <div className="absolute -bottom-32 left-1/4 h-72 w-72 rounded-full bg-white/5 blur-3xl" aria-hidden="true" />
          <div className="relative mx-auto max-w-7xl">
            ...
            <div className="grid gap-10 lg:grid-cols-12 lg:items-end">
              <div className="space-y-6 lg:col-span-8">
                <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider text-cream-tint backdrop-blur-sm">
                ...
                <h1 className="text-balance text-4xl font-extrabold leading-tight tracking-tight text-white sm:text-5xl lg:text-6xl">
                  Mengabdi, berkolaborasi, dan <span className="editorial-serif font-normal italic text-terracotta">bertumbuh bersama.</span>
                </h1>
              ...
              <div className="rounded-2xl border border-white/15 bg-white/10 p-6 backdrop-blur-sm lg:col-span-4">
                <div className="flex items-start gap-3"><MapPin size={20} className="mt-0.5 shrink-0 text-terracotta" aria-hidden="true" /><div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-cream-tint/50">Lokasi Pengabdian</p>
        ...
        <section className="mx-auto max-w-7xl px-5 py-16 md:py-20" aria-labelledby="tentang-tim-heading">
          <div className="grid gap-10 lg:grid-cols-12">
            <div className="lg:col-span-5"><p className="editorial-label">Tentang Tim</p><h2 id="tentang-tim-heading" className="mt-2 text-3xl font-extrabold tracking-tight text-charcoal md:text-4xl">Teknologi sebagai jembatan potensi lokal</h2></div>
        ...
        <section className="border-y border-sage-border bg-cream-tint px-5 py-16" aria-labelledby="kontribusi-heading">
          <div className="mx-auto max-w-7xl">
            <div className="mx-auto mb-10 max-w-2xl text-center"><p className="editorial-label">Kontribusi Kami</p><h2 id="kontribusi-heading" className="mt-2 text-3xl font-extrabold tracking-tight text-charcoal">Bertumbuh bersama Desa Loning</h2></div>
            <div className="grid gap-5 md:grid-cols-3">
              {contributions.map(({ icon: Icon, title, description: itemDescription }) => (
                <div key={title} className="group rounded-2xl border border-sage-border bg-cream-card p-6 transition-all hover:-translate-y-1 hover:shadow-lg">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-forest/10 text-forest transition-colors group-hover:bg-forest group-hover:text-white"><Icon size={22} aria-hidden="true" /></div>
                  <h3 className="mt-5 text-base font-bold text-charcoal">{title}</h3><p className="mt-2 text-xs leading-relaxed text-warm-gray">{itemDescription}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mx-auto max-w-7xl px-5 py-16 md:py-20">
          <div className="relative overflow-hidden rounded-3xl bg-charcoal p-8 text-cream-tint md:p-12">
            <div className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-terracotta/15 blur-3xl" aria-hidden="true" />
            <div className="relative flex flex-col gap-8 md:flex-row md:items-center md:justify-between">
              <div className="max-w-2xl"><div className="mb-3 flex items-center gap-2 text-terracotta"><Users size={18} aria-hidden="true" /><span className="text-xs font-bold uppercase tracking-widest">Di Balik Platform</span></div>
                <h2 className="text-2xl font-extrabold text-white md:text-3xl">LoningMaju dikembangkan oleh Stephen Michael</h2>
