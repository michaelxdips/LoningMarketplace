...
export default function UMKMDetailPage() {
  ...
  return <PublicPageShell>
    <article>
      ...
      <header className="mx-auto grid max-w-5xl gap-6 px-5 py-8 sm:py-12 lg:grid-cols-12 lg:items-center"><div className="overflow-hidden rounded-2xl border border-sage-border bg-cream-tint lg:col-span-6"><UMKMImage src={umkm.imageUrl} alt={umkm.altText || umkm.name} name={umkm.name} category={umkm.category} className="aspect-[4/3] h-full w-full object-cover"/></div><div className="lg:col-span-6"><p className="editorial-label">{umkm.category} · Profil Usaha</p><h1 className="text-balance mt-3 text-3xl font-extrabold tracking-[-0.04em] text-charcoal sm:text-5xl">{umkm.name}</h1><p className="mt-2 text-sm font-bold text-forest">Dikelola oleh {umkm.owner}</p><p className="mt-5 whitespace-pre-line text-sm leading-6 text-warm-gray">{umkm.description}</p><dl className="mt-6 grid gap-3 border-y border-sage-border py-5 text-sm"><div className="flex gap-3"><MapPin className="mt-0.5 shrink-0 text-terracotta" size={17}/><div><dt className="text-[10px] font-bold uppercase tracking-widest text-warm-gray">Alamat</dt><dd className="mt-1 leading-6 text-charcoal">{umkm.address}</dd></div></div>{(umkm.openingTime && umkm.closingTime) || umkm.workingHours ? <div className="flex gap-3"><Clock3 className="mt-0.5 shrink-0 text-terracotta" size={17}/><div><dt className="text-[10px] font-bold uppercase tracking-widest text-warm-gray">Jam operasional</dt><dd className="mt-1 text-charcoal">{umkm.openingTime && umkm.closingTime ? `${umkm.openingTime} – ${umkm.closingTime} WIB` : umkm.workingHours}</dd></div></div> : null}</dl><div className="mt-4 rounded-xl border border-sage-border bg-cream-bg p-3"><p className={`text-sm font-extrabold ${openStatus.kind === 'open' ? 'text-emerald-700' : 'text-charcoal'}`}>{openStatus.label}</p><p className="mt-1 text-xs text-warm-gray">{openStatus.detail}</p>{updatedLabel && <p className="mt-1 text-xs text-warm-gray">{updatedLabel}</p>}</div><div className="mt-6 flex flex-wrap gap-3"><button id="umkm-page-inquiry" disabled={umkm.isContactValid === false} onClick={() => setInquiryOpen(true)} className="focus-ring touch-target inline-flex items-center gap-2 rounded-lg bg-forest px-5 py-3 text-xs font-bold uppercase tracking-wider text-white disabled:opacity-50"><MessageSquare size={15}/> Hubungi via WhatsApp</button><ShareButton title={umkm.name} text={`Lihat profil ${umkm.name} di Loning Maju.`}/></div>{umkm.isContactVerificationFresh && <p className="mt-4 flex items-center gap-1.5 text-xs font-medium text-forest"><ShieldCheck size={14}/> Kontak telah diverifikasi pengelola.</p>}</div></header>
