...
import { ApiError, getProducts, getUMKM, PUBLIC_DETAIL_STALE_TIME } from '../lib/api';
import { formatPrice } from '../lib/price';
import { buildLocalBusinessJsonLd, usePageMetadata } from '../lib/seo';
...
export default function UMKMDetailPage() {
  ...
  return <PublicPageShell>
    <article>
      <nav aria-label="Breadcrumb" className="mx-auto max-w-5xl px-5 pt-6 text-xs text-warm-gray">
        <ol className="flex flex-wrap items-center gap-1.5">
          ...
          <li>
            <Link to="/#umkm" className="focus-ring rounded hover:text-forest">Direktori UMKM</Link>
          </li>
          <li aria-hidden="true" className="text-sage-border">/</li>
          <li>
            <span aria-current="page" className="font-semibold text-charcoal">{umkm.name}</span>
          </li>
        </ol>
      </nav>
      <header className="mx-auto grid max-w-5xl gap-6 px-5 py-8 sm:py-12 lg:grid-cols-12 lg:items-center"><div className="overflow-hidden rounded-2xl border border-sage-border bg-cream-tint lg:col-span-6"><UMKMImage src={umkm.imageUrl} alt={umkm.altText || umkm.name} name={umkm.name} category={umkm.category} className="aspect-[4/3] h-full w-full object-cover"/></div><div className="lg:col-span-6"><p className="editorial-label">{umkm.category} · Profil Usaha</p><h1 className="text-balance mt-3 font-serif text-3xl font-semibold leading-tight tracking-tight text-charcoal sm:text-5xl">{umkm.name}</h1><p className="mt-2 text-sm font-bold text-forest">Dikelola oleh {umkm.owner}</p><p className="mt-5 whitespace-pre-line text-sm leading-6 text-warm-gray">{umkm.description}</p><dl className="mt-6 grid gap-3 border-y border-sage-border py-5 text-sm"><div className="flex gap-3"><MapPin className="mt-0.5 shrink-0 text-terracotta" size={17}/><div><dt className="text-[11px] font-bold uppercase tracking-widest text-warm-gray">Alamat</dt><dd className="mt-1 leading-6 text-charcoal">{umkm.address}</dd></div></div>{(umkm.openingTime && umkm.closingTime) || umkm.workingHours ? <div className="flex gap-3"><Clock3 className="mt-0.5 shrink-0 text-terracotta" size={17}/><div><dt className="text-[11px] font-bold uppercase tracking-widest text-warm-gray">Jam operasional</dt><dd className="mt-1 text-charcoal">{umkm.openingTime && umkm.closingTime ? `${umkm.openingTime} – ${umkm.closingTime} WIB` : umkm.workingHours}</dd></div></div> : null}</dl><div className="mt-4 rounded-xl border border-sage-border bg-cream-bg p-3"><p className={`text-sm font-extrabold ${openStatus.kind === 'open' ? 'text-emerald-700' : 'text-charcoal'}`}>{openStatus.label}</p><p className="mt-1 text-xs text-warm-gray">{openStatus.detail}</p>{updatedLabel && <p className="mt-1 text-xs text-warm-gray">{updatedLabel}</p>}</div><div className="mt-6 flex flex-wrap gap-3"><button id="umkm-page-inquiry" disabled={umkm.isContactValid === false} onClick={() => setInquiryOpen(true)} className="focus-ring touch-target inline-flex items-center gap-2 rounded-lg bg-forest px-5 py-3 text-xs font-bold uppercase tracking-wider text-white disabled:opacity-50"><MessageSquare size={15}/> Hubungi via WhatsApp</button><ShareButton title={umkm.name} text={`Lihat profil ${umkm.name} di Loning Maju.`}/></div>{umkm.isContactVerificationFresh && <p className="mt-4 flex items-center gap-1.5 text-xs font-medium text-forest"><ShieldCheck size={14}/> Kontak telah diverifikasi pengelola.</p>}</div></header>
      <section className="mx-auto max-w-7xl px-5 pb-16"><BusinessLocation umkmName={umkm.name} address={umkm.address} latitude={umkm.latitude} longitude={umkm.longitude} /></section>
      <section aria-labelledby="catalog-title" className="border-t border-sage-border bg-cream-card"><div className="mx-auto max-w-7xl px-5 py-20"><div className="mb-9 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="editorial-label">Etalase usaha</p><h2 id="catalog-title" className="mt-2 break-words font-serif text-3xl font-semibold tracking-tight text-charcoal">Produk yang dipublikasikan</h2></div><p className="text-xs text-warm-gray">{productsQuery.data?.length ?? 0} item katalog</p></div>{productsQuery.isPending ? <p className="text-sm text-warm-gray">Memuat katalog…</p> : productsQuery.isError ? <p className="text-sm text-terracotta">Katalog belum dapat dimuat.</p> : productsQuery.data?.length ? <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{productsQuery.data.map((product) => <Link key={product.id} to={`/produk/${encodeURIComponent(product.slug)}`} className="focus-ring group overflow-hidden rounded-xl border border-sage-border bg-cream-bg transition-card hover:-translate-y-1 hover:shadow-md"><ProductImage src={product.imageUrl} alt={product.altText || product.name} className="aspect-[4/3] w-full object-cover transition-transform duration-500 group-hover:scale-[1.02]"/><div className="p-5"><p className="text-[10px] font-bold uppercase tracking-widest text-terracotta">{product.category}</p><h3 className="mt-2 text-lg font-bold text-charcoal">{product.name}</h3><p className="mt-4 text-sm font-bold text-forest">{formatPrice(product.price, 'Hubungi pelaku usaha')}</p></div></Link>)}</div> : <div className="rounded-xl border border-dashed border-sage-border p-10 text-center text-sm text-warm-gray">Belum ada produk yang dipublikasikan. Hubungi pelaku usaha untuk informasi terbaru.</div>}</div></section>
