import { Component, lazy, StrictMode, Suspense, useLayoutEffect, useRef, type ErrorInfo, type ReactNode } from 'react';
...
class LazyRouteBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
  ...
  componentDidCatch(_error: Error, _info: ErrorInfo) {
    /* Keep route chunk failures recoverable. */
  }
  render() {
    return this.state.failed ? (
      <main className="grid min-h-dvh place-items-center p-6">
        <div className="text-center">
          <h1 className="text-xl font-extrabold">Halaman tidak dapat dimuat</h1>
          <button
            className="mt-4 rounded-xl bg-forest px-4 py-2.5 text-sm font-bold text-white"
...
const queryClient = new QueryClient({
  defaultOptions: { queries: { refetchOnWindowFocus: false, retry: shouldRetryApiRequest } },
});
setUnauthorizedHandler(() => {
  try {
    localStorage.removeItem('loning_session_token');
  } catch {
    /* ignore */
  }
  void queryClient.cancelQueries({ queryKey: ['auth'] });
  queryClient.removeQueries({ queryKey: ['manage'] });
  queryClient.removeQueries({ queryKey: ['admin'] });
  queryClient.setQueryData(['auth', 'session'], null);
  queryClient.setQueryData(['auth', 'csrf'], null);
});
                  ...
                  <Route element={<PasswordGuard />}>
                    ...
                    <Route element={<CapabilityGuard capabilities={['dashboard:view']} />}>
                      <Route path="/dashboard" element={<DashboardShell />}>
                        ...
                        <Route
                          element={
                            <CapabilityGuard capabilities={['umkms:manage-location-all', 'umkms:manage-location-own']} />
                          }
                        >
