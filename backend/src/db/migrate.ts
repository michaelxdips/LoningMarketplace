...
const finalMigrationTag = '0009_repair_public_integrity';
const migrationSteps = ['preparePublicIntegrity', preparationMigrationTag, finalMigrationTag, 'assertPublicIntegrity'] as const;
...
try {
  await dialect.migrate(migrations.slice(0, preparationMigrationIndex), migrationSession, { migrationsFolder });
  await preparePublicIntegrity(database.db);
  await dialect.migrate(migrations.slice(preparationMigrationIndex), migrationSession, { migrationsFolder });
