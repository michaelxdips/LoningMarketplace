...
export async function seedUsers(db: PostgresJsDatabase<typeof schema>, cryptoHashPassword: (password: string) => Promise<string>, developmentPassword: string) {
  ...
  const users = [
    ...
    { id: USERS.admin2, email: 'admin2@local.test', username: 'admin2', displayName: 'Admin Tambahan', passwordHash: hash, role: 'admin' as const, isActive: true, mustChangePassword: false, createdAt: SEED_DATES.old, updatedAt: SEED_DATES.old },
