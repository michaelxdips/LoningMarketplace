...
async function createAsset(
  db: PostgresJsDatabase<typeof schema>, storage: MediaStorage, sourcePath: string,
  ...
  await db.insert(schema.mediaAssets).values({
    ...
    checksumSha256: createHash('sha256').update(card).digest('hex'), cardStorageKey: cardKey, thumbnailStorageKey: thumbKey,
...
export async function seedMedia(db: PostgresJsDatabase<typeof schema>) {
  ...
  for (let index = 0; index < products.length; index++) {
    ...
    await createAsset(
      db, storage, path.join(sourceDir, `product-${String(fixtureIndex).padStart(2, '0')}.jpg`),
  ...
  for (let index = 0; index < umkmIds.length; index++) {
    ...
    await createAsset(
      db, storage, path.join(sourceDir, `umkm-${String(fixtureIndex).padStart(2, '0')}.jpg`),
