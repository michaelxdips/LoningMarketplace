...
try {
   ...
   const username = input.BOOTSTRAP_ADMIN_EMAIL.split('@', 1)[0].toLowerCase().replace(/[^a-z0-9._-]/g, '-').replace(/^-+|-+$/g, '').slice(0, 30);
   if (username.length < 3) throw new Error('BOOTSTRAP_ADMIN_EMAIL must produce a username with at least 3 characters');
   const created = await repository.transaction(async (tx) => { const user = await tx.createUser({ email: input.BOOTSTRAP_ADMIN_EMAIL, username, displayName: input.BOOTSTRAP_ADMIN_DISPLAY_NAME, role: 'admin', passwordHash: await security.hashPassword(input.BOOTSTRAP_ADMIN_PASSWORD), mustChangePassword: true }); await tx.addAudit({ actorUserId: user.id, action: 'user.created', entityType: 'user', entityId: user.id, metadata: { source: 'bootstrap' } }); return user; });
