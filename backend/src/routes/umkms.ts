...
export async function umkmRoutes(app: FastifyInstance, repository: Repository) {
  ...
  app.get<{ Params: { id: string } }>('/umkms/:id', async (request, reply) => { const parsed = idSchema.safeParse(request.params.id); if (!parsed.success) return reply.code(400).send(error('Invalid UUID', 'VALIDATION_ERROR')); const item = await repository.getUMKM(request.params.id); return item ? { data: item } : reply.code(404).send(error('UMKM not found', 'NOT_FOUND')); });
}
