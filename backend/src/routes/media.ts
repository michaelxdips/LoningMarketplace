...
import { canManageMedia, hasCapability } from '../auth/policy.js';
...
export async function mediaRoutes(app: FastifyInstance, repository: Repository, guards: ReturnTypeGuards, storage: MediaStorage, env: AppEnv, id: () => string) {
  app.post('/manage/media/images', { preHandler: guards.secured }, async (request, reply) => {
    if (!hasCapability(request.auth!.user.role, 'media:manage-all') && !hasCapability(request.auth!.user.role, 'media:manage-own')) return reply.code(403).send(error('Capability is not assigned to this role', 'FORBIDDEN'));
  ...
  app.get<{ Params: { id: string } }>('/manage/media/images/:id', { preHandler: guards.authenticate }, async (request, reply) => { if (!uuid.safeParse(request.params.id).success) return reply.code(400).send(error('Invalid UUID', 'VALIDATION_ERROR')); const asset = await repository.getMediaAsset(request.params.id); if (!asset) return reply.code(404).send(error('Media asset not found', 'NOT_FOUND')); if (!canManageMedia(request.auth!.user.role, request.auth!.user.id, asset.createdByUserId)) return reply.code(403).send(error('Media ownership required', 'FORBIDDEN')); return { data: output(asset, storage) }; });
  app.patch<{ Params: { id: string } }>('/manage/media/images/:id', { preHandler: guards.secured }, async (request, reply) => { const parsed = z.strictObject({ altText: z.string().trim().max(500).nullable() }).safeParse(request.body); if (!parsed.success) return reply.code(400).send(error('Invalid media update', 'VALIDATION_ERROR')); const asset = await repository.getMediaAsset(request.params.id); if (!asset) return reply.code(404).send(error('Media asset not found', 'NOT_FOUND')); if (!canManageMedia(request.auth!.user.role, request.auth!.user.id, asset.createdByUserId)) return reply.code(403).send(error('Media ownership required', 'FORBIDDEN')); const updated = await repository.transaction(async (tx) => { const value = await tx.updateMediaAltText(asset.id, parsed.data.altText); await tx.addAudit({ actorUserId: request.auth!.user.id, action: 'media.updated', entityType: 'media_asset', entityId: asset.id, ...info(request) }); return value; }); return { data: output(updated, storage) }; });
  app.delete<{ Params: { id: string } }>('/manage/media/images/:id', { preHandler: guards.secured }, async (request, reply) => { const asset = await repository.getMediaAsset(request.params.id); if (!asset) return reply.code(404).send(error('Media asset not found', 'NOT_FOUND')); if (!canManageMedia(request.auth!.user.role, request.auth!.user.id, asset.createdByUserId)) return reply.code(403).send(error('Media ownership required', 'FORBIDDEN')); if (await repository.mediaReferenceCount(asset.id)) return reply.code(409).send(error('Media asset is still referenced', 'MEDIA_REFERENCED')); await repository.transaction(async (tx) => { await tx.deleteMediaAsset(asset.id); await tx.addAudit({ actorUserId: request.auth!.user.id, action: 'media.deleted', entityType: 'media_asset', entityId: asset.id, ...info(request) }); }); return { data: { deleted: true } }; });
}
