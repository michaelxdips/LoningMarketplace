...
export async function buildApp(env: AppEnv, repository: Repository, dependencies: AppDependencies = {}) {
  ...
  if (env.NODE_ENV === 'development' && media instanceof FilesystemMediaStorage) { await mkdir(media.root, { recursive: true }); await app.register(fastifyStatic, { root: media.root, prefix: '/media/', decorateReply: false, dotfiles: 'ignore', list: false, setHeaders: (reply) => { reply.header('Access-Control-Allow-Origin', env.CORS_ORIGIN); reply.header('Cross-Origin-Resource-Policy', 'cross-origin'); reply.header('Cache-Control', 'public, max-age=3600'); } }); }
