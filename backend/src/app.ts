...
import type { Repository } from './routes/repository.js';
...
import { createMediaStorage, FilesystemMediaStorage } from './media/storage.js';
import { mediaRoutes } from './routes/media.js';

const errorEnvelope = (message: string, code: string) => ({ error: { message, code } });
export type AppDependencies = { security?: Security; now?: () => Date; id?: () => string; storage?: import('./media/storage.js').MediaStorage };
export async function buildApp(env: AppEnv, repository: Repository, dependencies: AppDependencies = {}) {
  ...
  if (env.NODE_ENV === 'development' && media instanceof FilesystemMediaStorage) { await mkdir(media.root, { recursive: true }); await app.register(fastifyStatic, { root: media.root, prefix: '/media/', decorateReply: false }); }
  await app.register(async (api) => { await healthRoutes(api, repository); await umkmRoutes(api, repository); await productRoutes(api, repository); await authRoutes(api, repository, guards, crypto, env, now); await adminRoutes(api, repository, guards, crypto, now); await manageRoutes(api, repository, guards, now, id); await mediaRoutes(api, repository, guards, media, env, id); }, { prefix: '/api' });
